            ...
            if cell.cell_type == "code":
                ...
                if "create_engine" in cell.source and ".connect()" in cell.source:
                    ...
                    if cell.source[0] == "#":
                        ...
                        out.write(
                            f"[!] Missing question number comment identifier on first line of the cell containing SQL query: {cell.source} \n\n"
                        )
                        continue

                    if (
                        question_tag in question_tags_seen
                    ):  # Make sure no duplicate questions are graded
                        continue
                    else:
                    ...
                    if (
                        "outputs" in cell
                    ):  # Check whether the notebook has been executed
                        try:
                            with open(question_tag + ".pkl", "rb") as ao:
                                actual_output = pickle.load(ao)
                        except FileNotFoundError:
                            out.write(
                                f"[!] Missing/Incorrect pickle file name for question {question_number}\n"
                            )
                            print(
                                f"Found #q{question_number} tag but couldn't open student's pkl. Continuing..."
                            )
                            continue

                        try:
                            with open(
                                f"answers/answers/pkls/p{str(question_number)}.pkl",
                                "rb",
                            ) as eo:
                                expected_output = pickle.load(eo)
                        except FileNotFoundError:
                            out.write(
                                f"[!!] Could not open answers/q{question_number}.pkl"
                            )
                            print(
                                f"[!] Could not open answers/q{question_number}.pkl. Continuing"
                            )
                            continue

                        score_val = assert_frame_equal_extended_diff(
                            actual_output,
                            expected_output,
                            out,
                            question_tag,
                        ...
                        if (
                            int(question_number) == 23 and score_val == 0
                        ):  ## Q23 autograder fix
                            try:
                                with open(
                                    f"answers/answers/pkls/p23-s2.pkl", "rb"
                                ) as eo:
                                    expected_output_2 = pickle.load(eo)
                                total_score += assert_frame_equal_extended_diff(
                                    actual_output,
                                    expected_output_2,
                                    out,
                                    question_tag,
                        ...
                        out.write(
                            f"Question {question_number}: 0 points (NB did not have an output)\n"
                        )
